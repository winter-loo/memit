...
<div
  class="min-h-screen flex flex-col items-center justify-center p-6 font-display bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100"
>
  <div class="w-full max-w-[400px] space-y-10">
    <div class="flex flex-col items-center space-y-6">
      <!-- Logo -->
      <div
        class="w-12 h-12 bg-white dark:bg-card-dark border border-slate-200 dark:border-slate-800 rounded-full flex items-center justify-center shadow-sm"
      >
    ...
    >
      {#if errorMsg}
        <div class="text-red-500 text-sm text-center bg-red-50 dark:bg-red-900/10 py-2 rounded-lg">
          {errorMsg}
        </div>
      {/if}

      <div class="space-y-2">
        <label for="email" class="text-sm font-semibold text-slate-900 dark:text-white"
          >Email address</label
        >
        <input
          type="email"
      ...
      {#if view !== 'forgot'}
        <div class="space-y-2">
          <div class="flex items-center justify-between">
            <label for="password" class="text-sm font-semibold text-slate-900 dark:text-white"
              >Password</label
            >
            {#if view === 'login'}
              <button
