<script lang="ts">
  ...
  let { size = 24, class: className = '', animated = false }: Props = $props();
</script>

<svg
  class={`anki-icon ${className}`}
  class:anki-icon--animated={animated}
  role="img"
  width={size}
  height={size}
  viewBox="0 0 24 24"
...
<style>
  ...
  .anki-icon--animated .anki-star-large {
    animation: anki-send 1.25s infinite cubic-bezier(0.4, 0, 0.2, 1);
  }

  .anki-icon--animated .anki-star-small {
    animation: anki-receive 1.25s infinite cubic-bezier(0.4, 0, 0.2, 1);
  }
  ...
  @keyframes anki-send {
    ...
    20% {
      opacity: 1;
      transform: translate(0, 0) scale(1.08) rotate(-4deg);
    }
    48% {
      opacity: 0.42;
      transform: translate(1.6px, -1.6px) scale(0.88) rotate(5deg);
    }
    68% {
      opacity: 0.7;
      transform: translate(0.6px, -0.6px) scale(0.95) rotate(2deg);
    }
  }

  @keyframes anki-receive {
    0%,
    22% {
      opacity: 0.38;
      transform: translate(-1.8px, 1.8px) scale(0.74) rotate(0deg);
    }
    42% {
      opacity: 0.56;
      transform: translate(-0.9px, 0.9px) scale(0.86) rotate(-6deg);
    }
    62% {
      opacity: 1;
      transform: translate(0, 0) scale(1.18) rotate(5deg);
    }
    82% {
      opacity: 1;
